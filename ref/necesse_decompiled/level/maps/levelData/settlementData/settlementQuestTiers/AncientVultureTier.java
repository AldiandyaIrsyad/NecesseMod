...
public class AncientVultureTier extends SimpleSettlementQuestTier {
   ...
   public AncientVultureTier() {
      super("ancientvulture");
   }

   public Quest getTierCompleteQuest(SettlementLevelData var1) {
      return new HaveKilledMobsSettlementQuest("ancientvulture", 1);
   }
   ...
   static {
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("mummysbandage", 1);
      });
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("magicsand", 1);
      });
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("caveoyster", 1);
      });
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("capturedspirit", 1);
      });
      tierCompleteRewards = new LootTable(new LootItemInterface[]{new LootItem("recallflask"), addedQuestRewards});
      raiderStats = new TicketSystemList();
      raiderStats.addObject(100, new RaiderConstructor("ivysword", "demonichelmet", "demonicchestplate", "demonicboots", 675, 9, 20, 40));
      raiderStats.addObject(50, new RaiderConstructor("ivybow", "ivyhelmet", "ivychestplate", "ivyboots", 525, 6, 20, 40));
      raiderStats.addObject(25, new RaiderConstructor("voidapprentice", "voidmask", "voidrobe", "voidboots", 450, 5, 20, 40));
   }
}
