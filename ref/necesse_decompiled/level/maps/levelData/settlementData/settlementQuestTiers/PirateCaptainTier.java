...
public class PirateCaptainTier extends SimpleSettlementQuestTier {
   ...
   public PirateCaptainTier() {
      super("piratecaptain");
   }

   public Quest getTierCompleteQuest(SettlementLevelData var1) {
      return new HaveKilledMobsSettlementQuest("piratecaptain", 1);
   }
   ...
   static {
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("pegleg", 1);
      });
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("eyepatch", 1);
      });
      tierQuests.add((var0) -> {
         return new DeliverItemsSettlementQuest("rumbottle", 1);
      });
      tierCompleteRewards = new LootTable(new LootItemInterface[]{new LootItem("coinpouch"), addedQuestRewards});
      raiderStats = new TicketSystemList();
      raiderStats.addObject(100, new RaiderConstructor("cutlass", "demonichelmet", "demonicchestplate", "demonicboots", 825, 10, 25, 40));
      raiderStats.addObject(50, new RaiderConstructor("ivybow", "quartzhelmet", "quartzchestplate", "quartzboots", 675, 8, 25, 40));
      raiderStats.addObject(25, new RaiderConstructor("quartzstaff", "voidhat", "voidrobe", "voidboots", 600, 6, 25, 40));
   }
}
