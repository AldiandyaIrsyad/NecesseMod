...
public class MilkHusbandryMobLevelJob extends EntityLevelJob<HusbandryMob> {
   public SettlementHusbandryZone zone;

   public MilkHusbandryMobLevelJob(HusbandryMob var1, SettlementHusbandryZone var2) {
      super((Entity)var1);
      this.zone = var2;
   }

   public MilkHusbandryMobLevelJob(LoadData var1) {
      super(var1);
   }
   ...
   public static <T extends MilkHusbandryMobLevelJob> JobSequence getJobSequence(EntityJobWorker var0, final FoundJob<T> var1) {
      final InventoryItem var2 = new InventoryItem("bucket");
      if (((HusbandryMob)((MilkHusbandryMobLevelJob)var1.job).target).canMilk(var2)) {
         GameMessage var3 = ((HusbandryMob)((MilkHusbandryMobLevelJob)var1.job).target).getLocalization();
         LocalMessage var4 = new LocalMessage("activities", "milking", "target", var3);
         final GameLinkedListJobSequence var5 = new GameLinkedListJobSequence(var4);
         var5.add(new InteractMobActiveJob<HusbandryMob>(var0, var1.priority, (HusbandryMob)((MilkHusbandryMobLevelJob)var1.job).target, (GameTileRange)var1.handler.tileRange.apply(var0.getLevel()), (var2x) -> {
            return !((MilkHusbandryMobLevelJob)var1.job).isRemoved() && ((MilkHusbandryMobLevelJob)var1.job).isValid() && var2x.canMilk(var2);
         }, ((MilkHusbandryMobLevelJob)var1.job).reservable, var2) {
            public ActiveJobResult onInteracted(HusbandryMob var1x) {
               ArrayList var2x = new ArrayList();
               var1x.onMilk(var2, var2x);
   ...
   public static JobTypeHandler.JobStreamSupplier<? extends MilkHusbandryMobLevelJob> getJobStreamer() {
      return (var0, var1) -> {
         ...
         return var2.getLevel().entityManager.mobs.streamInRegionsShape(var5.getRangeBounds(var4), 0).filter((var1x) -> {
            ...
            return ((HusbandryMob)var0x).milkJob;
         }).filter(Objects::nonNull).filter(MilkHusbandryMobLevelJob::isValid);
      };
   }
}
