...
public class SparklerProjectile extends Projectile {
   private boolean hitMob = false;

   public SparklerProjectile() {
   }

   public SparklerProjectile(Level var1, float var2, float var3, float var4, float var5, int var6, GameDamage var7, Mob var8) {
      this.setLevel(var1);
      this.x = var2;
      this.y = var3;
   ...
   public void spawnSprayParticles(int var1) {
      ...
      for(int var3 = 0; var3 < var1; ++var3) {
         float var4 = this.x + GameRandom.globalRandom.floatGaussian() * 4.0F;
         float var5 = this.y + GameRandom.globalRandom.floatGaussian() * 4.0F;
         ...
         float var12 = GameRandom.globalRandom.getFloatBetween(0.2F, 1.0F);
         ...
         this.getLevel().entityManager.addParticle(var4, var5, var2.next()).fadesAlphaTime(0, var17).color(new Color(240, 180, 57)).sizeFadesInAndOut(15, 20, 100, 0).height((ParticleOption.HeightGetter)var19).rotates().moves((var4x, var5x, var6x, var7x, var8x) -> {
            if (var19.currentHeight > var9 && (!this.removed() || !this.hitMob)) {
               var21.tick(var4x, var5x, var6x, var7x, var8x);
            }

         }).sprite(GameResources.magicSparkParticles.sprite(GameRandom.globalRandom.nextInt(4), 0, 22)).givesLight(40.0F, 0.7F).lifeTime(var18);
      }
   ...
   public void doHitLogic(Mob var1, LevelObjectHit var2, float var3, float var4) {
      ...
      if (this.isServer() && var1 != null) {
         ActiveBuff var5 = new ActiveBuff("onfire", var1, 5.0F, this.getOwner());
