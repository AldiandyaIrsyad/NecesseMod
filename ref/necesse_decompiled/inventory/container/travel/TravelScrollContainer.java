...
public class TravelScrollContainer extends TravelContainer {
   public TravelScrollContainer(NetworkClient var1, int var2, Packet var3) {
   ...
   public void travelTo(IslandData var1, int var2) {
      if (this.client.isClient()) {
         ...
         if (var3.getLevelIdentifier().isIslandPosition()) {
            if (this.client.playerMob.getInv().removeItems(ItemRegistry.getItem("travelscroll"), 1, false, false, false, "travelscroll") > 0) {
            ...
            GameLog.warn.println(var3.getName() + " tried to use travel scroll from non island level");
            this.close();
         }
